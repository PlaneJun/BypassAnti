...
bool PEFile::AppendSection(std::string section_name, uint32_t size, uint32_t chrs, PIMAGE_SECTION_HEADER* newSec)
{
	...
	pSectionHeader[segCount].PointerToRawData = pSectionHeader[segCount - 1].PointerToRawData + FileAlign(pSectionHeader[segCount - 1].SizeOfRawData);
	...
	m_data.insert_data(pSectionHeader[segCount].PointerToRawData, section_data.data(), section_data.size());
...
bool PEFile::WriteToFile(std::string filepath)
{
	...
	fwrite((const char*)m_data.data(), 1, m_data.size(), f);
	fclose(f);
	return true;
}
...
bool PEFile::WriteBytes(uint32_t pos, void* buf, size_t size)
{
	...
	memcpy(&m_data[Rva2Foa(pos)], buf, size);
	return true;
}
...
bool PEFile::ReadBytes(uint32_t pos, void* buf, size_t size)
{
	...
	memcpy(buf,&m_data[Rva2Foa(pos)], size);
	return true;
}
